# [First non-repeating character in a stream](https://practice.geeksforgeeks.org/problems/first-non-repeating-character-in-a-stream1216/0)

**Level:** `Medium`.

**Topic:** `Hash`, `Linked List`, `Queue`.

Given an input stream of <strong>A </strong>of <strong>n</strong>&nbsp;characters consisting only of lower case alphabets. The task is to find the first non repeating character, each time a character is inserted to the stream. If there is no such character then append '#' to the answer.</span><br>
&nbsp;</p>

<p><span><strong>Example 1:</strong></span></p>

<pre><span><strong>Input: </strong>A = "aabc"
...
<pre><span><strong>Input: </strong>A = "zz"
...
<p><span><strong>Expected Time Complexity:&nbsp;</strong>O(26 * n)<br>
